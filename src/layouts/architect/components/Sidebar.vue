...
export default {
    ...
    computed: {
        ...
        menu() {
            if (this.$store.getters.isAdmin) return adminMenu;

            let m = R.clone(menu);
            if (this.organization && this.organization.id === 1) {
                m = R.reject(R.propEq('title', 'Marketplace'), m);
                m = m.reduce((arr, obj) => {
                    if (obj.title === 'Assets Portfolio') {
                        const o = R.clone(obj);
                        o.child = R.reject(R.propEq('title', 'Contracts'), o.child);
                        arr.push(o);
                    } else {
                        arr.push(obj);
                    }
